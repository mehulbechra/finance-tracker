...
export default function TransactionItem({
  ...
  const typesMap = {
    Income: { icon: HandCoins, colors: "text-green-500 dark:text-green-400" },
    Expense: { icon: Wallet, colors: "text-red-500 dark:text-red-400" },
    Investment: {
      icon: Landmark,
      colors: "text-yellow-500 dark:text-yellow-400",
    },
    Saving: { icon: PiggyBank, colors: "text-indigo-500 dark:text-indigo-400" },
  };
  ...
  return (
    <div className="w-full flex items-center">
      <div className="flex items-center mr-4 grow">
        <IconComponent className={`${colors} mr-2 w-4 h-4 hidden md:flex`} />
      ...
      <div className="min-w-[100px] flex justify-end items-center">
        <Link
        ...
        <TransactionItemRemoveButton id={id} onDelete={onDelete} />
      </div>
    </div>
  );
}
